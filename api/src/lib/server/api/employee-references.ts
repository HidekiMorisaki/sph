...
export async function employeeReferenceErrors(tx: Prisma.TransactionClient, data: ReferenceInput): Promise<ApiErrorDetail[]> {
	const checks = await Promise.all([
		...
		data.groupId === null || tx.employeeGroup.count({ where: { id: data.groupId, deletedAt: null } }).then((count) => count === 1),
	...
	return checks.flatMap((active, index) => active ? [] : [{ field: fields[index], reason: 'Select an active option.' }]);
}
