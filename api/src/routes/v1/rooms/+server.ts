...
const sortFields = ['id', 'code', 'name', 'branch', 'notes', 'createdAt', 'updatedAt'] as const;
...
export async function GET({ locals, url }: import('./$types').RequestEvent) {
	...
	const where: Prisma.RoomWhereInput = { deletedAt: null, ...(search ? { OR: [{ code: { contains: search, mode: 'insensitive' } }, { name: { contains: search, mode: 'insensitive' } }, { notes: { contains: search, mode: 'insensitive' } }, { branch: { is: { name: { contains: search, mode: 'insensitive' }, deletedAt: null } } }] } : {}) };
