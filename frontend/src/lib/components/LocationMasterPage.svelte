<script lang="ts">
	...
	let form=$state({name:'',postalCode:'',prefecture:'',city:'',streetAddress:'',buildingName:'',phoneNumber1:'',phoneNumber1Label:'',phoneNumber2:'',phoneNumber2Label:'',faxNumber1:'',faxNumber1Label:'',faxNumber2:'',faxNumber2Label:'',managerEmployeeId:'',deputyManagerEmployeeId:'',branchId:'',roomId:'',notes:''});
	...
	const minTableWidth=$derived(tab==='branches'?1400:tab==='rooms'?820:980);
...
{#snippet branchFormSections()}
	<FormSection title="Basic information" framed>{@render nameField()}</FormSection>
...
<AssetManagementShell {title} active="">
	...
	<section class="panel" class:branch-panel={tab==='branches'}>
		<MasterList bind:this={list} {endpoint} columns={tab==='branches'?[{key:'name',label:'Name',width:13,value:(item:Branch)=>item.name},{key:'responsiblePerson',label:'Responsible person',width:16,sortable:false,cell:responsiblePeopleCell},{key:'postalCode',label:'Postal code',width:10,value:(item:Branch)=>item.postalCode??''},{key:'prefecture',label:'Prefecture',width:10,value:(item:Branch)=>item.prefecture??''},{key:'city',label:'City',width:12,value:(item:Branch)=>item.city??''},{key:'streetAddress',label:'Street address',width:18,value:(item:Branch)=>item.streetAddress??''},{key:'phoneNumber1',label:'Phone',width:12,value:(item:Branch)=>contactLines(item.phoneNumber1,item.phoneNumber1Label,item.phoneNumber2,item.phoneNumber2Label)},{key:'faxNumber1',label:'Fax',width:9,value:(item:Branch)=>contactLines(item.faxNumber1,item.faxNumber1Label,item.faxNumber2,item.faxNumber2Label)}]:columns} {title} {canManage} {minTableWidth} initialSortBy="name" onDetail={tab==='branches'?(item,trigger)=>showDetail(item as Branch,trigger):undefined} onEdit={(item,trigger)=>edit(item as Branch|Room|Storage,trigger)} onDelete={(item)=>remove(item as Branch|Room|Storage)} />
	</section>
...
{#if detailBranch}
	...
	<DetailModal title="Branch detail" titleId="branch-detail-title" closeLabel="Close branch detail" returnFocus={returnFocus} compact dialogClass="branch-detail-dialog" onClose={closeDetail}>
		<section class="app-detail-section"><h3>Basic information</h3><dl class="app-detail-grid"><div><dt>Name</dt><dd>{detailBranch.name}</dd></div></dl></section>
...
{#if canManage && formOpen}
	<div class="app-modal-backdrop" role="presentation"><dialog bind:this={dialogElement} class="location-dialog app-modal app-modal--compact" class:branch-dialog={tab==='branches'} open aria-modal="true" aria-labelledby="location-dialog-title">
