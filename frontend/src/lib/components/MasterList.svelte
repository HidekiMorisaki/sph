...
<div class="master-scroll" onscroll={() => closeMenu()}>
	<table style={`min-width:${minTableWidth}px`}>
		...
		<tbody>{#if loading}<tr><td class="empty" colspan={columns.length + (showActions ? 1 : 0)}>{loadingLabel}</td></tr>{:else if error}<tr><td class="empty" colspan={columns.length + (showActions ? 1 : 0)}>{error}</td></tr>{:else if !items.length}<tr><td class="empty" colspan={columns.length + (showActions ? 1 : 0)}>{emptyLabel}</td></tr>{:else}{#each items as item (item.id)}<tr>{#each columns as column}<td>{#if column.cell}{@render column.cell(item)}{:else}{column.value?.(item) ?? ''}{/if}</td>{/each}{#if showActions}<td class="actions-cell"><button class="kebab-button master-kebab" type="button" aria-label={`Actions for ${actionLabel?.(item) ?? item.name ?? item.displayName ?? item.code ?? item.id}`} aria-haspopup="menu" aria-expanded={menuItem?.id === item.id} onclick={(event) => openMenu(event, item)}><svg viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><circle cx="3" cy="8" r="1.4"/><circle cx="8" cy="8" r="1.4"/><circle cx="13" cy="8" r="1.4"/></svg></button></td>{/if}</tr>{/each}{/if}</tbody>
	</table>
</div>
