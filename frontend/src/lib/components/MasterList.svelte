<script lang="ts">
	...
	type Column = { key: string; label: string; value: (item: any) => string | number | null | undefined; width?: number; primary?: boolean };
	let { endpoint, columns, title, listHeading, description = 'Sortable, searchable, paginated.', initialSortBy = 'code', pageSizeStorageKey, minTableWidth = 720, canManage = false, onDetail, onEdit, onDelete }: {
		endpoint: string; columns: Column[]; title: string; listHeading?: string; description?: string; initialSortBy?: string; pageSizeStorageKey?: string; minTableWidth?: number; canManage?: boolean;
	...
	let showActions = $derived(Boolean(onDetail || canManage));
	...
	let pageNumbers = $derived(Array.from({ length: Math.min(5, pageCount) }, (_, i) => Math.max(1, Math.min(page - 2, pageCount - 4)) + i));
	...
	$effect(() => { endpoint; initialSortBy; storageKey; if (typeof window !== 'undefined') { page = 1; search = ''; appliedSearch = ''; sortBy = initialSortBy; sortOrder = 'asc'; const stored = Number(localStorage.getItem(storageKey)); pageSize = sizes.includes(stored) ? stored : 10; untrack(() => void refresh()); } });
	onMount(() => {
...
<header class="master-header"><h2>{listHeading ?? `All ${title.toLowerCase()}`}</h2><p>{description}</p></header>
...
<div class="master-scroll" onscroll={() => closeMenu()}>
	<table style={`min-width:${minTableWidth}px`}>
		<colgroup>{#each columns as column}<col style={`width:${column.width ?? (showActions ? 95 : 100) / columns.length}%`} />{/each}{#if showActions}<col style="width:5%" />{/if}</colgroup>
		<thead><tr>{#each columns as column}<th aria-sort={activeSortBy === column.key ? sortOrder === 'asc' ? 'ascending' : 'descending' : 'none'}><button type="button" class="sort-button" onclick={() => changeSort(column.key)}>{column.label}<span class="sort-indicator" class:ascending={activeSortBy === column.key && sortOrder === 'asc'} class:descending={activeSortBy === column.key && sortOrder === 'desc'} aria-hidden="true"></span></button></th>{/each}{#if showActions}<th><span class="sr-only">Row actions</span></th>{/if}</tr></thead>
		<tbody>{#if loading}<tr><td colspan={columns.length + (showActions ? 1 : 0)}>Loading...</td></tr>{:else if error}<tr><td colspan={columns.length + (showActions ? 1 : 0)}>{error}</td></tr>{:else if !items.length}<tr><td colspan={columns.length + (showActions ? 1 : 0)}>No items found.</td></tr>{:else}{#each items as item (item.id)}<tr>{#each columns as column}<td>{#if column.primary}<strong>{column.value(item) ?? ''}</strong>{:else}{column.value(item) ?? ''}{/if}</td>{/each}{#if showActions}<td class="actions-cell"><button class="kebab-button master-kebab" type="button" aria-label={`Actions for ${item.name ?? item.displayName ?? item.code}`} aria-haspopup="menu" aria-expanded={menuItem?.id === item.id} onclick={(event) => openMenu(event, item)}><svg viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><circle cx="3" cy="8" r="1.4"/><circle cx="8" cy="8" r="1.4"/><circle cx="13" cy="8" r="1.4"/></svg></button></td>{/if}</tr>{/each}{/if}</tbody>
	</table>
</div>
<footer class="master-footer"><p>Showing {first}–{last} of {total}</p><nav aria-label={`${title} pages`}><button type="button" aria-label="First page" disabled={page === 1} onclick={() => changePage(1)}>&lt;&lt;</button><button type="button" aria-label="Previous page" disabled={page === 1} onclick={() => changePage(page - 1)}>&lt;</button>{#each pageNumbers as number}<button type="button" aria-current={number === page ? 'page' : undefined} onclick={() => changePage(number)}>{number}</button>{/each}<button type="button" aria-label="Next page" disabled={page === pageCount} onclick={() => changePage(page + 1)}>&gt;</button><button type="button" aria-label="Last page" disabled={page === pageCount} onclick={() => changePage(pageCount)}>&gt;&gt;</button></nav></footer>
</div>
{#if menuItem}<div class="menu-popover master-menu" role="menu" style={`top:${menuTop}px;left:${menuLeft}px`}><button type="button" role="menuitem" disabled={!onDetail} onclick={() => { const item = menuItem; const trigger = menuTrigger; closeMenu(); if (item) onDetail?.(item, trigger); }}>Detail</button><button type="button" role="menuitem" disabled={!canManage || !onEdit} onclick={() => { const item = menuItem; const trigger = menuTrigger; closeMenu(); if (item && canManage) onEdit?.(item, trigger); }}>Edit</button><div class="menu-separator"></div><button type="button" class="delete-item" role="menuitem" disabled={!canManage || !onDelete} onclick={() => { const item = menuItem; closeMenu(); if (item && canManage) void onDelete?.(item); }}>Delete</button></div>{/if}
...
<style>
	...
	.master-header{flex:none;padding:14px 16px;border-bottom:1px solid var(--border-light)}
	...
	@media(max-width:700px){.master-list{max-height:calc(100dvh - 180px)}.master-toolbar,.master-footer{align-items:stretch;flex-direction:column}.search-box{width:100%}}
</style>
