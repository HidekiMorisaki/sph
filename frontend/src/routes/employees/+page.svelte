...
{#snippet pageActions()}<AddButton label="Add employee" disabled={!canManageEmployees} onclick={create} />{/snippet}
...
<AssetManagementShell title="Employees" active="Employees">
	...
	<MasterPageHeader title="Employees" actions={pageActions} />
...
<style>
	...
	.employees-page{display:flex;height:calc(100dvh - 124px);min-height:0;flex-direction:column}.export-button{display:inline-flex;align-items:center;justify-content:center;height:32px;padding:0 12px;background:var(--surface)!important;color:var(--text-secondary)!important;border:1px solid var(--border);border-radius:4px;box-shadow:var(--shadow);font-size:12.5px;font-weight:500;line-height:1;transition:background 120ms,border-color 120ms,color 120ms,box-shadow 120ms;white-space:nowrap}.export-button:hover{background:var(--surface-secondary)!important;color:var(--text)!important}.export-button:disabled{cursor:wait;opacity:.6}.export-button:focus{outline:none}.export-button:focus-visible{outline:2px solid #1abb9c;outline-offset:2px}.notice{flex:none;margin:0 0 14px;color:#169f85}.employee-cell{display:flex;align-items:center;gap:8px}.avatar{display:grid;flex:0 0 24px;height:24px;place-items:center;background:#1abb9c;color:#fff;border-radius:50%;font-size:9px;font-weight:600}.employee-cell strong{display:block;color:var(--text);font-weight:500}.employee-cell small{display:block;color:var(--muted);font-size:11px}@media(max-width:700px){.export-button{width:100%}}
