<script lang="ts">
	...
	async function complete() {
		...
		if (password.length < 12 || !/[a-z]/.test(password) || !/[A-Z]/.test(password) || !/\d/.test(password)) {
			error = 'Use at least 12 characters with uppercase, lowercase, and numbers.';
			return;
		}
		loading = true;
		try {
...
<main class="setup-page"><section class="setup-card">
	...
	{:else}<h1>Set up your account</h1><p>Choose a password to activate your account. The invitation link can be used only once.</p>
		<form onsubmit={(event) => { event.preventDefault(); void complete(); }}>
			<label>New password<input bind:value={password} type="password" autocomplete="new-password" required minlength="12" maxlength="1024" /></label>
			<label>Confirm password<input bind:value={confirmPassword} type="password" autocomplete="new-password" required minlength="12" maxlength="1024" /></label>
...
<style>
	...
	form{display:grid;gap:15px}label{display:grid;gap:6px;font-size:13px;font-weight:600}input{width:100%;padding:10px;border:1px solid var(--border);border-radius:4px;background:var(--bg);color:var(--text)}
