...
{:else}<main class="auth"><form novalidate onsubmit={(event) => { event.preventDefault(); void login(); }}><h1>{productName}</h1><p>Sign in to manage workplace assets.</p><label>Username or email <span class="required" aria-hidden="true">*</span><input bind:this={identifierInput} bind:value={identifier} autocomplete="username" maxlength="254" aria-invalid={!!identifierError} aria-describedby={identifierError?'identifier-error':undefined} oninput={()=>identifierError=''} />{#if identifierError}<small id="identifier-error" class="field-error">{identifierError}</small>{/if}</label><label>Password <span class="required" aria-hidden="true">*</span><input bind:this={passwordInput} bind:value={password} type="password" autocomplete="current-password" aria-invalid={!!passwordError} aria-describedby={passwordError?'login-password-error':undefined} oninput={()=>passwordError=''} />{#if passwordError}<small id="login-password-error" class="field-error">{passwordError}</small>{/if}</label><button type="submit" disabled={isSubmitting}>{isSubmitting ? 'Signing in…' : 'Sign in'}</button>{#if message}<p role="alert">{message}</p>{/if}</form></main>{/if}
...
<style>
  ...
  input{height:36px;padding:0 12px;border:1px solid var(--border);border-radius:4px;background:var(--bg);color:var(--text)}input[aria-invalid='true']{border-color:var(--danger)}.required,.field-error{color:var(--danger)}.field-error{font-size:11px;font-weight:400}
