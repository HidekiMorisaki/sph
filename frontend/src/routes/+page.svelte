...
{:else if user?.mustChangeCredentials}<main class="auth"><form onsubmit={(event) => { event.preventDefault(); void completeOnboarding(); }}><h1>Complete your account</h1><p>Set a new password to continue.</p><p>Email: {user.email}</p><label>New password<input bind:value={password} type="password" autocomplete="new-password" required minlength="12" /></label><button type="submit" disabled={isSubmitting}>Save account</button>{#if message}<p role="alert">{message}</p>{/if}</form></main>
{:else if user}<div class:collapsed={sidebarCollapsed} class="shell"><AppSidebar collapsed={sidebarCollapsed} {user} currentPath="/" home onLogout={logout} /><section class="workspace" aria-labelledby="dashboard-title"><AppHeader collapsed={sidebarCollapsed} onToggleSidebar={toggleSidebar} breadcrumbs={breadcrumbsForPath('/', 'Operations')} /><div class="page-title"><p>OPERATIONS</p><h1 id="dashboard-title">Welcome back, {user.name ?? user.username}</h1><span>Manage your workplace assets from one place.</span></div><div class="cards"><article><small>ASSET OVERVIEW</small><b>Ready to manage</b><span>Keep inventory current</span></article><article><small>ATTENTION</small><b>No critical alerts</b><span>Review assignments regularly</span></article><article><small>ACCOUNT</small><b>{user.role}</b><span>{user.email ?? 'Email not set'}</span></article></div><section class="panel"><h2>Management</h2><p>Use the sidebar to maintain employees, locations and equipment.</p></section></section></div>
{:else}<main class="auth"><form onsubmit={(event) => { event.preventDefault(); void login(); }}><h1>{productName}</h1><p>Sign in to manage workplace assets.</p><label>Username or email<input bind:value={identifier} autocomplete="username" required maxlength="254" /></label><label>Password<input bind:value={password} type="password" autocomplete="current-password" required /></label><button type="submit" disabled={isSubmitting}>{isSubmitting ? 'Signing in…' : 'Sign in'}</button>{#if message}<p role="alert">{message}</p>{/if}</form></main>{/if}
...
<style>
  ...
  input{padding:10px;border:1px solid var(--border);border-radius:4px;background:var(--bg);color:var(--text)}
